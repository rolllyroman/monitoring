...
def doclub_getRoomList(redis, session):
    ...
    for roomNum in roomLists:
        ...
        gameName, dealer, playerCount, maxPlayer, gameid, baseScore, ruleText, gameState, room_club_number, curGameCount, maxGameCount = redis.hmget(ROOM2SERVER%(roomNum), ('gameName', 'dealer', 'playerCount', 'maxPlayer', 'gameid','baseScore', 'ruleText', "gameState", "club_number", "curGameCount", "maxGameCount"))
    ...
    return {'code':0, 'roomData':roomDatas, "data": {"playerNumber": playersNumber, "active": onlineNumber, "roomNumber": roomlistNumber, "roomActiveNumber": roomActiveNumber, "club_number": club_number, "RoomCards": userRoomCards}}
...
def club_into(redis, session):
    ...
    if redis.exists(CLUB_ATTR % club_number):
        ...
        return {"code":0, "msg": '成功', "data": {"playerNumber": playersNumber, "active": onlineNumber, "roomNumber": roomlistNumber, "roomActiveNumber": roomActiveNumber, "club_number": club_number}, "prompt": msg, "isManager": isManager, "clubroomCards": roomCards, "curPlayerCards": curPlayerCards}
...
def settting_auto_create_allow(redis, session):
    ...
    if settingAllow == 1:
        ...
        roomCards = redis.get(USER4AGENT_CARD % (ag, id)) or 0
        if int(roomCards) < 100:
            return {"code": 1, "msg": "俱乐部所属群主砖石不足100，请充值后开启。 "}
