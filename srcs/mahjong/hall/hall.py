...
def do_hallLogin(redis,session):
    ...
    if reAccount:
        ...
        userInfo = {'name':name,'isTrail':int(isTrail),'shop':int(shop),'group_id':ag,'account':reAccount, 'passwd':rePasswd,'uid':id,'group_id_auto':defaultgid,'group_id_default':dgid ,'OnA':ACTIVITIVE_SWITCH, 'notice_open':NEW_NOTICE ,'roomCardClub':roomCardClub}
        joinNum = ''
        id = userTable.split(':')[1]
