...
@admin_app.post("/member/clearAgentStatus")
@checkLogin
def clearAgentStatus(redis, session):
    """ 清除用户公会状态

    """
    user_id = request.forms.get("user_id", '').strip()
    agent_id = request.forms.get("agent_id", '').strip()
    topAgent = request.forms.get("topAgent", '').strip()
    level = int(request.forms.get("level", 3).strip())
    DEFAULT_AGENT = '540302'
    clearUserAgentStatus(user_id, agent_id, topAgent, level, redis, default=DEFAULT_AGENT)
