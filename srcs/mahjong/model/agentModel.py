...
def qrCodeImageSave(url, other='000000'):
    """ 生成二维码图片
    ...
    imageFile = "%s-%s.png" %(int(time.time()), other)
    qr.add_data(url)
    qr.make(fit=True)
    img = qr.make_image()
    img.save("mahjong/static/" + imageFile)
    return imageFile
...
def get_agent_list_infos(redis,sub_agent_ids,agent_own_accesses,lang):
    ...
    for subId in sub_agent_ids:
        ...
        aType,valid,reg_date,account,roomCard,aId,isTrail,isCreate,recharge,auto_check,create_auth,open_auth= \
                        redis.hmget(agentTable,AGENT_LIST_DIS)

        securyKeys = redis.hget(EXCHANGE_AGENT_FIND, subId)
        if not securyKeys:
            securyKeys = SecretKeys(redis)
            redis.hset(EXCHANGE_AGENT_FIND, subId, securyKeys)

        if not account or aType == '4':
            continue
        managerNumber = len(redis.smembers("agent:%s:manager:member:set" % subId))

        gid = redis.hget('agents:id:%s'%aId,'parent_id')
        ...
        agInfo = {
                ...
                "aType"      :       aType,
                "securyKeys" :       securyKeys,
                "managerNumber":     managerNumber
        }
        agInfo["exchangeIntoCode"] = ''
        if aId >= 2:
            exchangeIntoCode = redis.hget(EXCHANGE_AGENT_CODE, subId)
            if not exchangeIntoCode:
                exchangeIntoCode = SecretKeysInto(redis, subId)
            agInfo["exchangeIntoCode"] = exchangeIntoCode
        #获取操作权限
        agInfo['op'] = []
        for access in access_module.ACCESS_AGENT_LIST:
            if access.url in agent_own_accesses:
                ...
                elif "create" in access.url:
                    agInfo['op'].append({'url': "%s/%s" % (access.url, aId), 'method': "POST", 'txt': access.getTxt(lang)})
                else:
...
def getAgListInfos(redis,session,agentId,condition,lang):
    ...
    if agentId == god_id:
        subIds = []
        for x in redis.smembers('sysadmin:id:set'):
            agentId = str(x)
            parentTable = AGENT_CHILD_TABLE%(agentId)
            subIds += list(redis.smembers(parentTable))
        subIds = set(subIds)
...
def get_agent_active(redis,agentId,startDate,endDate):
    ...
    while endDate >= startDate:
        ...
        for child_id in self_child_ids:
            ...
            pid,type = redis.hmget('agents:id:%s'%agentInfo['id'],'parent_id','type')
            agentInfo['gid'] = pid
            if type == '2':
                agentInfo['gid'] = redis.hget('agents:id:%s'%pid,'parent_id')
