...
def main():
    ...
    senderMgr.registerCommands(
        ...
        Packer(robot_pb2.S_S_HOTRELOAD, robot_pb2.S_S_Hotreload),
        )
    )

    ws = create_connection("ws://192.168.16.50:19990")
    resps = robot_pb2.S_S_Robotaddr()
    resps.roomId = "924555"
    resps.num = 3
    resps.level = 0
    resps.type = 0
    # print(ws.send_binary(senderMgr.pack(resp)))
    print(ws.send_binary(senderMgr.pack(resps)))
